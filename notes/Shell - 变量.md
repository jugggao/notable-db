...
脚本只能将变量输出到子进程，即在这个脚本中所调用的命令或程序。在命令行中调用的脚本不能够将变量回传给命令行环境，即子进程不能将变量传回给父进程，所以子 Shell 无法修改继承的变量。
...
每当 Shell 启动时，都会创建出与其环境对应的 Shell 变量。改变或增加 Shell 环境变量会使 Shell 更新其自身的环境。子进程会继承父进程的环境变量。
